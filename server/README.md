...
Переезжаем на VPS в России (Timeweb, 109.73.199.217, 152-ФЗ): технический адрес сервера из DNS
(свой домен — по желанию), nginx + Let's Encrypt.
...
bash setup.sh install [адрес]             # пакеты, код, настройки со старого сервера, HTTPS; сервис ещё не запущен
...
Без адреса `install` берёт технический адрес сервера (обратная запись DNS). До `move` — попросить ABCP разрешить
